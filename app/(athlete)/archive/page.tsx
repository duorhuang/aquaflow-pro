...
export default function TrainingArchivePage() {
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center text-center p-6">
                <div className="space-y-4 max-w-xs">
                    ...
                    <p className="text-sm text-muted-foreground">无法识别您的队员身份，请重新登录。</p>
                    <button onClick={() => router.push('/login')} className="w-full bg-primary text-white py-3 rounded-xl font-bold">返回登录页</button>
                </div>
            </div>
        );
    }
    ...
    // 1. Standalone plans (no date limit — show all)
        ...
        <div className="min-h-screen bg-background pb-24">
            ...
            <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border p-4">
                <div className="flex items-center justify-between max-w-lg mx-auto">
                    <button onClick={() => router.back()} className="p-2 hover:bg-secondary/50 rounded-lg">
            ...
            <main className="p-4 max-w-lg mx-auto space-y-6">
                {/* Tab Switcher */}
                <div className="grid grid-cols-2 gap-2 bg-card/30 border border-border rounded-xl p-1.5">
                    <button
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-card/30 p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] text-muted-foreground uppercase">总训练日</p>
                                <p className="text-lg font-bold text-white">{totalTrainingDays}</p>
                            </div>
                            <div className="bg-card/30 p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] text-muted-foreground uppercase">已出勤</p>
                                <p className="text-lg font-bold text-green-400">{attendedDays}</p>
                            </div>
                            <div className="bg-card/30 p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] text-muted-foreground uppercase">周计划</p>
                                <p className="text-lg font-bold text-blue-400">{weeklyPlanCount}</p>
                            </div>
                        </div>

                        {/* Month Navigation */}
                        <div className="flex items-center justify-between bg-card/30 border border-border rounded-xl p-3">
                        ...
                        {monthEntries.length > 0 ? (
                            <div className="space-y-3">
                                {monthEntries.map((entry, idx) => {
                                    ...
                                    return (
                                        <div key={`${entry.date}-${idx}`} className="bg-card/30 border border-border rounded-xl overflow-hidden">
                                            <div
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-white">
                                                            {d.toLocaleDateString('zh-CN', { weekday: 'short' })} 训练
                                                        </p>
                                                        {entry.type === 'weekly' && entry.weeklyPlanTitle && (
                                                            <p className="text-[10px] text-blue-400">周计划: {entry.weeklyPlanTitle}</p>
                                                        )}
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {entry.plan.focus && <span className="text-[10px] text-muted-foreground">{entry.plan.focus}</span>}
                            ...
                            <div className="text-center py-16 bg-card/30 border border-dashed border-border rounded-3xl">
                                ...
                                <p className="text-sm text-muted-foreground">{viewMonth.toLocaleDateString('zh-CN', { year: 'numeric', month: 'long' })} 没有训练记录</p>
                            </div>
                        )}
                    </>
                )}
                ...
                {activeTab === 'feedback' && (
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="bg-card/30 p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] text-muted-foreground uppercase">总反馈</p>
                                <p className="text-lg font-bold text-white">{totalFeedback}</p>
                            </div>
                            <div className="bg-card/30 p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] text-muted-foreground uppercase">教练批复</p>
                                <p className="text-lg font-bold text-green-400">{coachReplies}</p>
                            </div>
                            <div className="bg-card/30 p-4 rounded-2xl border border-border text-center">
                                <p className="text-[10px] text-muted-foreground uppercase">待回复</p>
                ...
                <div onClick={() => setExpanded(!expanded)} className="p-4 cursor-pointer hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[10px] text-muted-foreground">{d.toLocaleDateString('zh-CN')} · {bf.planId?.substring(0, 8)}...</p>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        {bf.tags && bf.tags.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                                {bf.tags.map((tag: string) => (
                                    <span key={tag} className="text-[10px] bg-white/10 text-muted-foreground px-2 py-0.5 rounded-full">{tag}</span>
                                ))}
                            </div>
                        )}
                    </div>
                ...
                <div onClick={() => setExpanded(!expanded)} className="p-4 cursor-pointer hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[10px] text-muted-foreground">{wf.weekStart} 周 · 提交于 {d.toLocaleDateString('zh-CN')}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            {wf.isReplied && <span className="text-[10px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full">已批复</span>}
                            {!wf.isReplied && <span className="text-[10px] bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded-full">待批复</span>}
                            <ChevronRight className={cn("w-4 h-4 text-muted-foreground transition-transform", expanded && "rotate-90")} />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="p-4 border-t border-border bg-black/20 space-y-3">
                        {wf.summary && (
                            <div className="bg-primary/5 border border-primary/20 p-3 rounded-lg">
                                <p className="text-[10px] text-primary font-bold mb-1">我的总结</p>
                        ...
                        {wf.dailyFeedbacks?.filter((df: any) => df.reflection).map((df: any) => (
                            <div key={df.id} className="bg-white/5 p-3 rounded-lg">
                                <p className="text-[10px] text-muted-foreground font-bold mb-1">{df.date} 日记</p>
                ...
                <div onClick={() => setExpanded(!expanded)} className="p-4 cursor-pointer hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-[10px] text-muted-foreground">{tf.reminderMessage?.substring(0, 30)}{tf.reminderMessage?.length > 30 ? '...' : ''}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            {tf.coachReply && <span className="text-[10px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full">已批复</span>}
                            <ChevronRight className={cn("w-4 h-4 text-muted-foreground transition-transform", expanded && "rotate-90")} />
                        </div>
                    </div>
                </div>
                {expanded && (
                    <div className="p-4 border-t border-border bg-black/20 space-y-3">
                        ...
                        <div className="bg-white/5 p-3 rounded-lg">
                            <p className="text-[10px] text-muted-foreground font-bold mb-1">我的回复</p>
