...
import { announcementRepo } from '@/lib/repos';
...
export async function GET(req: Request) {
  return handleAnyAuth(req, async () => {
    ...
    const announcements = await announcementRepo.list({ group: group || undefined, archived });
...
export async function PUT(request: Request) {
  return handleCoach(request, async () => {
    ...
    const data = flattenPayload(await request.json());
    const announcement = await announcementRepo.toggleStar(id, Boolean(data.isStarred));
    return NextResponse.json(announcement, { headers: V12_FINGERPRINT });
  });
}
